...
def convert_markdown_to_html(markdown_text: str) -> Tuple[str, str]:
    ...
    """
    # Split content and bibliography — support both French and English headers
    bib_header = '## Bibliographie' if '## Bibliographie' in markdown_text else '## Bibliography'
    parts = markdown_text.split(bib_header)
    content_md = parts[0]
    bibliography_md = parts[1] if len(parts) > 1 else ""

    # Extract citation-to-URL map from bibliography BEFORE converting content,
    # so inline [N] citations can become clickable links to sources
    citation_urls = _extract_citation_urls(bibliography_md)

    # Convert content (everything except bibliography)
    content_html = _convert_content_section(content_md, citation_urls)
...
def _extract_citation_urls(bibliography_md: str) -> dict:
    """
    Parse bibliography text to build a map of citation number -> URL.
    Supports formats like:
      [1] Author (Year). "Title". Pub. https://url (Retrieved: date)
      [2] Author - https://url
    """
    urls = {}
    for match in re.finditer(r'\[(\d+)\].*?(https?://[^\s\)]+)', bibliography_md):
        num = match.group(1)
        url = match.group(2)
        urls[num] = url
    return urls
...
def _convert_content_section(markdown: str, citation_urls: dict = None) -> str:
    ...
    # Convert inline citations [N] to clickable links opening in new tab
    def _citation_to_link(match):
        num = match.group(1)
        if num in citation_urls:
            url = citation_urls[num]
            return f'<a href="{url}" target="_blank" rel="noopener noreferrer" class="citation">[{num}]</a>'
        return f'<span class="citation">[{num}]</span>'

    html = re.sub(r'\[(\d+)\]', _citation_to_link, html)
